...
import com.psybergate.resoma.project.api.ProjectService;
import com.psybergate.resoma.project.entity.Task;
import com.psybergate.resoma.time.api.TimeService;
...
public class TimeController {
    ...
    public ResponseEntity<TimeEntry> captureTimeEntry(@RequestBody @Valid TimeEntryDTO timeEntryDTO) {
    ...
    public ResponseEntity<TimeEntry> captureTimeEntry2(@RequestBody @Valid com.psybergate.resoma.time.dto.TimeEntryDTO timeEntryDTO) {
    ...
    public ResponseEntity<TimeEntry> updateTimeEntry(@PathVariable UUID timeEntryId, @RequestBody @Valid TimeEntryDTO timeEntryDTO) {
        TimeEntry timeEntry = buildTimeEntry(timeEntryDTO);
    ...
    public ResponseEntity<TimeEntry> submitTimeEntry(@PathVariable UUID timeEntryId,
                                                     @RequestBody @Valid TimeEntryDTO timeEntryDTO) {
        TimeEntry timeEntry = buildTimeEntry(timeEntryDTO);
        if (!timeEntryId.equals(timeEntry.getId()))
            throw new ValidationException("id in url path does not match time entry id in request body");
    ...
    public ResponseEntity<TimeEntry> approveTimeEntry(@PathVariable UUID timeEntryId, @RequestBody @Valid TimeEntryDTO timeEntryDTO) {
        TimeEntry timeEntry = buildTimeEntry(timeEntryDTO);
        if (!timeEntryId.equals(timeEntry.getId()))
            throw new ValidationException("id in url path does not match time entry id in request body");
    ...
    public ResponseEntity<TimeEntry> rejectTimeEntry(@PathVariable(name = "id") UUID entryId, @RequestBody @Valid TimeEntryDTO timeEntryDTO) {
        TimeEntry timeEntry = buildTimeEntry(timeEntryDTO);
